...
class FTS_Youtube_Feed_Free extends feed_them_social_functions
{
    ...
    function fts_youtube_func($atts) {
        ...
        if(isset($youtubeAPIkey) && $youtubeAPIkey !== '' || isset($youtubeAccessToken) && $youtubeAccessToken !== ''){
            ...
            $youtubeAPIkeyORtoken = isset($youtubeAccessToken) && $youtubeAccessToken !=='' ? 'access_token='.$youtubeAccessToken.'' : 'key='.$youtubeAPIkey.'';
            ...
            if (!isset($_GET['load_more_ajaxing']) && $youtubeAPIkey == '' && $youtubeAccessToken !== '') {
                ...
                if (time() > $expiration_time) {
                    ...
                    // print 'ajaxing now to refresh token';
                }
            }
            ...
            if (!isset($_GET['load_more_ajaxing'])) {
                ...
                if ($large_vid == 'yes' || $vids_in_row == '1') {
                    foreach ($videos->items as $post_data) {
                        ...
                        if($video_check !== 'set'){
                            $output .= '<div id="fts-yt-large-' . $username . '" class="fts-yt-large' . $wrap . '">';
                            $output .= '<div class="fts-yt-first-video">';

                            $large_vid_title = isset($large_vid_title) && $large_vid_title == 'yes' ? '<h2>' . $this->fts_youtube_title($post_data) . '</h2>' : '';
                            $output .= $large_vid_title;
                            ...
                            if($thumbs_play_in_iframe !== 'yes' || $thumbs_play_in_iframe !== 'no'){
                ...
                $output .= '<div ' . $columns . $force_columns . '' . $video_id_or_link_final . ' class="' . $fts_dynamic_class_name . ' fts-youtube-popup-gallery fts-instagram-inline-block-centered ' . $thumbgalleryClass . '" ' . $thumbs_wrap_color_final . ' >';
                ...
                if (isset($video_id_or_link) && $video_id_or_link !== '') {
                    ...
                    $output .= $this->fts_youtube_single_video_info($video_id_or_link, $youtubeAPIkeyORtoken);
            ...
            if ($vids_in_row !== '0' && $video_id_or_link == '') {
            ...
            if ($video_id_or_link == '') {
                ...
                if (!isset($_GET['load_more_ajaxing']) && is_plugin_active('feed-them-premium/feed-them-premium.php')) {
                    ...
                    $output .= 'slickremixImageResizing();'; // Reload our imagesizing function so the images show up proper
     ...
     * @since 2.3.2
     */
    function fts_youtube_single_video_info($videoID, $youtubeAPIkey) {
